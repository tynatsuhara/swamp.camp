...
export const Settings = {
    ...
    increaseMusicVolume: () => {
        const volume =
            settings.musicVolume === 1 ? 0 : Math.floor(settings.musicVolume * 100 + 10) / 100
    ...
    decreaseMusicVolume: () => {
        const volume =
            settings.musicVolume === 0 ? 1 : Math.floor(settings.musicVolume * 100 - 10) / 100
        settings.musicVolume = volume
        Music.setVolume(settings.musicVolume)
        saveSettings()
    },
    ...
    increaseSoundVolume: () => {
        const volume =
            settings.soundVolume === 1 ? 0 : Math.floor(settings.soundVolume * 100 + 10) / 100
        settings.soundVolume = volume
        saveSettings()
    },
    decreaseSoundVolume: () => {
        const volume =
            settings.soundVolume === 0 ? 1 : Math.floor(settings.soundVolume * 100 - 10) / 100
        settings.soundVolume = volume
        saveSettings()
    },
}
